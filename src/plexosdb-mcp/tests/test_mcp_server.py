"""Unit tests for the plexosdb-mcp MCP server implementation."""
...
from plexosdb_mcp import MCPServerState
import plexosdb_mcp.server as mcp_server
...
def test_main_prints_hint_when_launched_interactively(
    ...
    """main() prints usage hint to stderr when stdin is a TTY without --allow-tty."""
    monkeypatch.setattr(mcp_server.sys, "stdin", StubStdin(True))

    mcp_server.main()
    captured = capsys.readouterr()

    assert "must be launched by an MCP client" in captured.err
    assert "@modelcontextprotocol/inspector" in captured.err
    assert captured.out == ""
...
def test_main_cli_open_xml_session_requires_path(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--cli-command open-xml-session exits 1 with JSON error when --xml-path is omitted."""
    ...
    assert "--xml-path is required" in payload["error"]


def test_smoke_health_positional_subcommand(capsys: pytest.CaptureFixture[str]) -> None:
    """'health' positional subcommand prints JSON and exits — documented happy path."""
    mcp_server.main(["health"])
    out = capsys.readouterr().out.strip()
...
def test_smoke_health_positional_and_cli_command_are_equivalent(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Positional 'health' and --cli-command health produce identical output."""
    mcp_server.main(["health"])
    out_positional = json.loads(capsys.readouterr().out.strip())

    mcp_server.main(["--cli-command", "health"])
    out_legacy = json.loads(capsys.readouterr().out.strip())

    assert out_positional == out_legacy


def test_smoke_version_subcommand(capsys: pytest.CaptureFixture[str]) -> None:
    """'version' subcommand prints JSON with version fields."""
    ...
    assert "plexosdb_version" in payload
    assert "python" in payload


def test_smoke_version_accepts_json_flag(capsys: pytest.CaptureFixture[str]) -> None:
    """'version --json' is accepted and produces the same JSON output."""
    mcp_server.main(["version", "--json"])
    out = capsys.readouterr().out.strip()
...
def test_smoke_doctor_subcommand(capsys: pytest.CaptureFixture[str]) -> None:
    """'doctor' subcommand prints JSON with all checks passing in the test environment."""
    mcp_server.main(["doctor"])
    out = capsys.readouterr().out.strip()

    payload = json.loads(out)
    assert payload["ok"] is True
    check_names = {c["name"] for c in payload["checks"]}
    assert {"fastmcp", "plexosdb", "empty_session"} == check_names
    assert all(c["ok"] for c in payload["checks"])


def test_smoke_doctor_exits_1_when_check_fails(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_smoke_capabilities_subcommand(capsys: pytest.CaptureFixture[str]) -> None:
    """'capabilities' subcommand prints JSON with tool categories and subcommands."""
    mcp_server.main(["capabilities"])
    out = capsys.readouterr().out.strip()
    ...
    assert "capabilities" in payload["subcommands"]


def test_smoke_capabilities_accepts_json_flag(capsys: pytest.CaptureFixture[str]) -> None:
    """'capabilities --json' is accepted and produces the same JSON output."""
    mcp_server.main(["capabilities", "--json"])
    out = capsys.readouterr().out.strip()
...
def test_main_runtime_error_exits_1_with_json_on_stderr(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Unhandled runtime errors exit with code 1 and structured JSON on stderr."""
    monkeypatch.setattr(mcp_server.sys, "stdin", StubStdin(False))
